...
class ChromaCollection:
    ...
    def get(
        ...
        try:
            ...
            logger.debug(
                f"Retrieved {len(result['ids'])} document(s) " f"from collection '{self.name}'"
            )
            return result
        except Exception as e:
    ...
    def query(
        ...
        try:
            ...
            logger.debug(
                f"Query returned {len(result['ids'][0]) if result['ids'] else 0} "
                f"result(s) from collection '{self.name}'"
            )
            return result
        except Exception as e:
