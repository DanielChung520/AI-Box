...
def delete_dev_user_tasks(db, confirm: bool = False):
    ...
    try:
        ...
        for task in tasks:
            print(f"  - {task.get('title', task.get('task_id', 'N/A'))} (task_id: {task.get('task_id', 'N/A')})")
        ...
        if db.has_collection("file_metadata"):
            ...
            file_ids = [f.get('file_id') or f.get('_key') for f in files if f.get('file_id') or f.get('_key')]
