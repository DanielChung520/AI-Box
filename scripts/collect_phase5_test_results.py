...
def parse_pytest_output(output: str) -> Dict[str, Any]:
    ...
    # 解析測試詳情
    test_pattern = r"tests/integration/(phase\d+)/(test_\w+\.py)::(\w+)::(\w+)\s+(PASSED|FAILED|SKIPPED|ERROR)"
...
def main():
    ...
    for line in full_output.split("\n"):
        if "test_ollama_integration.py" in line:
            ...
            f"test_{x}" in line
            for x in ["llm_router", "multi_llm", "load_balancer", "failover"]
        ):
