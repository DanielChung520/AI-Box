...
def upload_file(file_path: Path, token: str) -> Optional[str]:
    ...
    try:
        with open(file_path, "rb") as f:
            ...
            response = requests.post(
                f"{API_BASE_URL}/files/upload", files=files, headers=headers, timeout=30
            )
            if response.status_code == 200:
                data = response.json()
...
def wait_for_processing(file_id: str, token: str, timeout: int) -> Dict[str, Any]:
    ...
    while time.time() - start_time < timeout:
        ...
        try:
            response = requests.get(
                f"{API_BASE_URL}/files/{file_id}/processing-status", headers=headers, timeout=30
            )
            ...
            print(
                f"  ⏱️  [{elapsed}s] 整體進度: {overall_progress}%, KG提取: {kg_progress}% ({kg_status_value})"
            )
    ...
    print(f"  ⏱️  {error_msg}")
    return {"status": "timeout", "error": error_msg, "total_time": timeout}
...
def get_kg_results(file_id: str, token: str) -> Dict[str, Any]:
    ...
    try:
        headers = {"Authorization": f"Bearer {token}"}
        response = requests.get(f"{API_BASE_URL}/files/{file_id}/kg", headers=headers, timeout=30)
        if response.status_code == 200:
