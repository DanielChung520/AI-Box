...
for i in range(120):  # 最多监控 10 分钟
    ...
    print(f"  分块: {chunking.get('status', 'unknown')} ({chunking.get('progress', 0)}%) - {chunking.get('chunk_count', 0)} 块")
    print(f"  向量化: {vectorization.get('status', 'unknown')} ({vectorization.get('progress', 0)}%)")
    print(f"  存储: {storage.get('status', 'unknown')} ({storage.get('progress', 0)}%) - {storage.get('vector_count', 0)} 向量")
    print(f"  圖譜: {kg_extraction.get('status', 'unknown')} ({kg_extraction.get('progress', 0)}%) - NER:{kg_extraction.get('entities_count', 0)} RE:{kg_extraction.get('relations_count', 0)} RT:{kg_extraction.get('triples_count', 0)}")
