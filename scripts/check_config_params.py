...
def scan_env_vars(project_root: Path) -> Dict[str, Set[str]]:
    ...
    for py_file in project_root.rglob("*.py"):
        ...
        if any(
            x in str(py_file)
            for x in ["venv", "node_modules", ".git", "__pycache__", "backup", "tests"]
        ):
            continue
        try:
            ...
            for match in re.finditer(r'os\.getenv\(["\']([^"\']+)["\']', content):
                ...
                if (
                    ...
                    and not param.startswith("TEST_")
                    and not param.startswith("SMOKE_")
                    and not param.startswith("SKIP_")
                ):
...
def main():
    ...
    categories = {
        ...
        "SeaweedFS": [
            ...
            "AI_BOX_SEAWEEDFS_FILER_ENDPOINT",
            "DATALAKE_SEAWEEDFS_S3_ENDPOINT",
            "DATALAKE_SEAWEEDFS_S3_ACCESS_KEY",
            "DATALAKE_SEAWEEDFS_S3_SECRET_KEY",
            "DATALAKE_SEAWEEDFS_USE_SSL",
            "DATALAKE_SEAWEEDFS_FILER_ENDPOINT",
        ],
        "安全": [
            "JWT_SECRET_KEY",
