#!/usr/bin/env python3
# 代碼功能說明: 將 KA-Agent Ontology（domain + major）導入 ArangoDB
# 創建日期: 2026-01-25 20:35 UTC+8
# 創建人: Daniel Chung
# 最後修改日期: 2026-01-25 22:58 UTC+8

"""將 KA-Agent Ontology 導入 ArangoDB（須先於上架知識庫前執行）
...
def _generate_ontology_key(type: str, name: str, version: str, tenant_id: str | None = None) -> str:
    """生成 Ontology 的 _key（與 ontology_store_service 一致）"""
...
def main() -> int:
    base = _project_root / "kag" / "ontology"
    domain_path = base / "ka-agent-domain.json"
    major_path = base / "ka-agent-major.json"
    ...
    if existing_major:
        ...
        create_m = OntologyCreate(
            ...
            compatible_domains=raw_m.get("compatible_domains", ["ka-agent-domain.json"]),
            tags=raw_m.get("tags", []),
