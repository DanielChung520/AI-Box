...
def update_phase5_stats(content: str, results: Dict[str, Any]) -> str:
    """更新階段五統計部分"""
    phase5_tests = {
        k: v for k, v in results.get("tests", {}).items() if k.startswith("IT-5")
    }
...
def update_phase5_in_progress_table(content: str, results: Dict[str, Any]) -> str:
    ...
    for test_id in ["IT-5.1", "IT-5.2", "IT-5.3", "IT-5.4"]:
        ...
        pattern = rf"(\| \| {re.escape(test_id)} \| {re.escape(test_descriptions[test_id])} \| ).*?(\|)"
...
def update_total_stats(content: str, results: Dict[str, Any]) -> str:
    ...
    # 計算總計
    total_tests = (
        phase1_total + phase2_total + phase3_total + phase4_total + phase5_total
    )
    total_passed = (
        phase1_passed + phase2_passed + phase3_passed + phase4_passed + phase5_passed
    )
    total_skipped = (
        phase1_skipped
        + phase2_skipped
