...
# -*- coding: utf-8 -*-
"""
代碼功能說明: MM-Agent 測試數據清理腳本
創建日期: 2026-01-25
創建人: Daniel Chung
最後修改日期: 2026-01-25 23:51 UTC+8

功能:
- 清理 ArangoDB 的 user_tasks、file_metadata、entities、relations（僅限 task_id=MM-Agent）
- 清理 Qdrant 的對應 file_* collections
- 清理 SeaweedFS 的 tasks/MM-Agent/ 目錄
...
使用方法:
    python3 scripts/cleanup_mm_agent_data.py [--dry-run] [--yes]

示例:
    python3 scripts/cleanup_mm_agent_data.py --dry-run
    python3 scripts/cleanup_mm_agent_data.py
    python3 scripts/cleanup_mm_agent_data.py --yes
"""
...
# 任務配置（可通過環境變數覆蓋）
TASK_ID = os.getenv("MM_AGENT_TASK_ID", "MM-Agent")
USER_ID = os.getenv("MM_AGENT_USER_ID", "systemAdmin")
...
def cleanup_arangodb() -> Dict[str, int]:
    ...
    file_ids = []  # 用於後續清理 entities/relations
    ...
    try:
        ...
        # 3. 清理 entities（根據 file_id 或 file_ids 過濾）
        try:
            if not file_ids:
                print_status("無 file_ids，跳過 entities 清理", "INFO")
            else:
        ...
        # 4. 清理 relations（根據 file_id 或 file_ids 過濾）
        try:
            if not file_ids:
                print_status("無 file_ids，跳過 relations 清理", "INFO")
            else:
                coll = db.collection("relations")
                cursor = db.aql.execute(
                    """
                    FOR r IN relations
                    FILTER r.file_id IN @file_ids OR (
                        r.file_ids != null AND LENGTH(INTERSECTION(r.file_ids, @file_ids)) > 0
                    )
                    RETURN r
                    """,
                    bind_vars={"file_ids": file_ids},
...
def cleanup_seaweedfs() -> Dict[str, int]:
    ...
    base_path = f"/buckets/{SEAWEEDFS_BUCKET}/tasks/{TASK_ID}/"

    try:
        # 列出目錄內容
        url = f"{filer_url}{base_path}"
        import base64
        ...
        entries = response.json().get("Entries") or response.json().get("entries") or []
        ...
        # 遞歸刪除文件
        # Filer API 回傳扁平的 Entries（FullPath），無 SubEntries；逐筆 DELETE
        for entry in entries:
            full_path = entry.get("FullPath", "")
            if not full_path:
                continue
            delete_url = f"{filer_url}{full_path}"
            delete_req = httpx.Request("DELETE", delete_url, headers={"Authorization": auth_header})
            try:
                ...
                if delete_response.status_code in (200, 202, 204):
                    ...
                    stats["errors"] += 1
                    print_status(f"刪除失敗: {full_path} ({delete_response.status_code})", "ERROR")
            except Exception as e:
                stats["errors"] += 1
...
def dry_run_cleanup() -> None:
    ...
    print(f"  /buckets/{SEAWEEDFS_BUCKET}/tasks/{TASK_ID}/")
...
def confirm_cleanup() -> bool:
    ...
    print(f"  - SeaweedFS: /buckets/{SEAWEEDFS_BUCKET}/tasks/{TASK_ID}/")
    print("\n此操作不可逆！")
    print("=" * 80)
