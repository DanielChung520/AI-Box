...
class _FakePermissionService:
    ...
    def check_file_access(
        ...
        *,
        ...
        _ = user
        _ = file_id
        if required_permission == Permission.FILE_UPDATE.value and not self.allow_update:
...
def app_client(monkeypatch: pytest.MonkeyPatch) -> TestClient:
    ...
    monkeypatch.setattr(docs_router, "get_metadata_service", lambda: fake_meta, raising=True)
    monkeypatch.setattr(docs_router, "get_file_permission_service", lambda: fake_perm, raising=True)

    # mock LLM output
    from llm.moe.moe_manager import LLMMoEManager

    async def _fake_generate(self: LLMMoEManager, prompt: str, **kwargs: Any) -> Dict[str, Any]:
        _ = self
        _ = kwargs
...
def test_doc_edit_preview_apply_and_versions(app_client: TestClient) -> None:
    ...
    state = None
    for _ in range(30):
        s = app_client.get(f"/api/v1/docs/edits/{request_id}", headers={"X-Tenant-ID": "default"})
    ...
    v = app_client.get("/api/v1/docs/files/f1/versions", headers={"X-Tenant-ID": "default"})
...
def test_doc_edit_base_version_mismatch(app_client: TestClient) -> None:
    ...
    for _ in range(30):
        ...
        if s.json()["data"]["status"] == "succeeded":
            break
    app_client.post(f"/api/v1/docs/edits/{request_id}/apply", headers={"X-Tenant-ID": "default"})
