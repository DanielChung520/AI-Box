...
class TestInternalAuth:
    ...
    async def test_authenticate_internal_agent_success(self):
        """測試內部 Agent 認證成功"""
        with patch("agents.services.auth.internal_auth.get_agent_registry") as mock_registry:
            # Mock Registry
...
class TestExternalAuth:
    ...
    async def test_authenticate_external_agent_success(self):
        """測試外部 Agent 認證成功"""
        with patch("agents.services.auth.external_auth.get_agent_registry") as mock_registry:
            mock_agent_info = AgentRegistryInfo(
