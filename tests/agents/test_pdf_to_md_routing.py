...
測試 pdf-to-md Agent 的語義路由能力（10 個場景：PDF2MD-001 ~ PDF2MD-010）
注意：所有場景都必須包含明確的轉換關鍵詞語義（轉換、轉為、轉成、轉換為、變成等）、PDF 關鍵字（.pdf）以及 Markdown 關鍵字（.md, Markdown）
"""
...
# pdf-to-md 測試場景定義（10 個場景，所有場景都包含明確的轉換關鍵詞語義、PDF 關鍵字和 Markdown 關鍵字）
PDF_TO_MD_SCENARIOS = [
    {
...
class TestPDFToMDRouting:
    ...
    async def test_pdf_to_md_routing(self, orchestrator, scenario):
        """測試 pdf-to-md Agent 語義路由"""
        ...
        md_keyword = scenario.get("md_keyword", "")
        ...
        print(f"Markdown 關鍵字: {md_keyword}")
        print(f"用戶輸入: {user_input}")
        print(f"預期 Agent: {expected_agent}")
        print(f"{'='*80}")

        # 驗證場景包含轉換關鍵詞語義
        conversion_keywords = ["轉換", "轉為", "轉成", "轉換為", "變成", "生成", "導出", "提取", "轉"]
        ...
        if not has_md_keyword:
            print(f"  ⚠️  警告: 場景 {scenario_id} 未包含明確的 Markdown 關鍵字")
        ...
        try:
            ...
            if called_md_editor and expected_agent == PDF_TO_MD_AGENT_ID:
                print("  ⚠️  警告: 錯誤調用了 md-editor（應該調用 pdf-to-md）")

            called_pdf_to_md = "pdf-to-md" in analysis_result.suggested_agents
            if not called_pdf_to_md and expected_agent == PDF_TO_MD_AGENT_ID:
                print("  ⚠️  警告: 未調用預期的 pdf-to-md Agent")
            ...
            all_passed = (
                ...
                and not (called_md_editor and expected_agent == PDF_TO_MD_AGENT_ID)
            )
            print("\n[測試總結]")
            if all_passed:
                print("  ✅ 所有驗證點通過")
            ...
            result = {
                ...
                if expected_agent == PDF_TO_MD_AGENT_ID
                else False,
...
def save_test_results() -> Path:
    ...
    output_file = output_dir / f"pdf_to_md_test_results_{timestamp}.json"
