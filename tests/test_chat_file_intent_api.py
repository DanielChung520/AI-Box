...
class _FakePermissionService:
    def check_task_file_access(
        self, *, user: User, task_id: str, required_permission: str
    ) -> None:
        return None
    ...
    def check_file_access(
        self, *, user: User, file_id: str, required_permission: str
    ) -> None:
        return None
...
def test_chat_creates_file_in_specified_folder_path(
    ...
    monkeypatch.setattr(
        chat_router, "_ensure_folder_path", lambda **_kwargs: "folder-xyz"
    )
    ...
    resp = app_client.post(
        ...
        json={
            "messages": [
                {"role": "user", "content": "請把以上內容整理成 docs/meeting.md"}
            ],
            "session_id": "s2",
            "task_id": "t1",
            "model_selector": {"mode": "auto"},
