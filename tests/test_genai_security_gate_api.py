...
def app_client(monkeypatch: pytest.MonkeyPatch) -> TestClient:
    ...
    async def _fake_chat(self: LLMMoEManager, *args: Any, **kwargs: Any) -> Dict[str, Any]:
...
class _FakePolicyGate:
    ...
    def is_model_allowed(self, provider: str, model_id: Optional[str]) -> bool:
        ...
        if self.allowed_providers and prov not in {p.lower() for p in self.allowed_providers}:
            return False
...
def test_g6_model_gate_manual_disallowed(
    ...
    class _FakeConfigResolver:
        def get_effective_policy_gate(self, *, tenant_id: str, user_id: str) -> _FakePolicyGate:
            _ = tenant_id
            _ = user_id
