...
class TestAgentPlatformIntegration:
    ...
    async def test_resource_access_control_integration(self):
        ...
        # 測試資源訪問
        assert (
            resource_controller.check_access("restricted-agent", ResourceType.MEMORY, "allowed-ns")
            is True
        )
        assert (
        ...
        assert (
            resource_controller.check_access("restricted-agent", ResourceType.TOOL, "tool1") is True
        )
        assert (
            resource_controller.check_access("restricted-agent", ResourceType.TOOL, "tool2")
            is False
        )
