            ...
            if upload_resp.status_code == 200:
                ...
                if uploaded:
                    ...
                    if config_resp.status_code == 200:
                        ...
                        print("當前配置:")
                        print(
                            f"  NER: {config_data.get('ner_model_type')} - {config_data.get('ner_model')}"
                        )
                        print(
                            f"  RE: {config_data.get('re_model_type')} - {config_data.get('re_model')}"
                        )
                        print(
                            f"  RT: {config_data.get('rt_model_type')} - {config_data.get('rt_model')}"
                        )
                        print()
                        ...
                        update_resp = client.put(
                            f"{base_url}{api_prefix}/config/kg_extraction",
                            headers=headers,
                            json={"config_data": config_data, "is_active": True},
                            timeout=30.0,
                        )
                        ...
                        while waited < max_wait:
                            ...
                            if status_resp.status_code == 200:
                                ...
                                if (
                                    ...
                                    or relations != last_relations
                                    or triples != last_triples
                                    or waited % 60 == 0
                                ):  # 每60秒至少顯示一次
                                    print(
                                        f"   [{waited:4d}s] "
