#!/usr/bin/env python3
# 代碼功能說明: 使用 Gemini 模型重新生成圖譜（帶時間記錄）
# 創建日期: 2026-01-04
# 創建人: Daniel Chung
# 最後修改日期: 2026-01-04

"""使用 Gemini 模型重新生成圖譜（帶時間記錄）

使用方法:
    python test_regenerate_kg_gemini.py <file_id>
"""
...
def login(username: str = "daniel@test.com", password: str = "test123") -> Optional[str]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            record_time(
                "登錄失敗", details={"HTTP狀態": response.status_code, "耗時": f"{login_time:.3f}秒"}
            )
            return None
...
def regenerate_graph(file_id: str, token: str) -> bool:
    """重新生成圖譜"""
    record_time("開始觸發圖譜重新生成", details={"File ID": file_id, "模型": "Gemini"})
...
def monitor_processing(file_id: str, token: str, max_wait: int = 600) -> bool:
    """監控處理進度（帶時間記錄）"""
    record_time("開始監控處理進度", details={"最多等待": f"{max_wait}秒", "模型": "Gemini"})
    start_time = time.time()
    last_progress = -1
    last_status = None
    ...
    while True:
        ...
        if kg_extraction_status == "processing" and kg_extraction_start is None:
            ...
            record_time("圖譜提取階段開始（Gemini）", details={"已用時": f"{elapsed:.2f}秒"})
        ...
        if overall_status == "completed":
            ...
            if kg_extraction_start:
                ...
                record_time("圖譜提取階段完成（Gemini）", details={"耗時": f"{kg_extraction_time:.2f}秒"})
...
def save_time_report(file_id: str, model: str = "Gemini"):
    """保存時間記錄報告"""
    report_file = project_root / f"docs/測試報告_圖譜重新生成_{model}_{file_id[:8]}.json"
    ...
    if "圖譜提取階段完成（Gemini）" in events and "圖譜提取階段開始（Gemini）" in events:
        report["階段耗時統計"]["圖譜提取"] = events["圖譜提取階段完成（Gemini）"] - events["圖譜提取階段開始（Gemini）"]
...
def main():
    if len(sys.argv) < 2:
        print("用法: python test_regenerate_kg_gemini.py <file_id>")
        sys.exit(1)

    file_id = sys.argv[1]

    print("=" * 60)
    print("使用 Gemini 模型重新生成圖譜（帶時間記錄）")
    print("=" * 60)
    print(f"File ID: {file_id}")
    print("模型: Gemini (gemini-pro)")
    ...
    # 5. 保存時間記錄報告
    report_file = save_time_report(file_id, "Gemini")
...
if __name__ == "__main__":
    try:
        ...
        print("\n\n⚠️  用戶中斷")
        if time_records:
            report_file = save_time_report("interrupted", "Gemini")
        ...
        traceback.print_exc()
        if time_records:
            report_file = save_time_report("error", "Gemini")
            print(f"已保存部分時間記錄: {report_file}")
        sys.exit(1)
