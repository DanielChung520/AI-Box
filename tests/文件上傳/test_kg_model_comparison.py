...
def verify_model_config(token: str, expected_model: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if kg_config and kg_config.config_data:
            ...
            if (
                actual_ner == expected_model
                and actual_re == expected_model
                and actual_rt == expected_model
            ):
                config_info["verified"] = True
                config_info["note"] = "✅ ArangoDB 配置與預期模型一致"
                record_time("配置驗證成功", details={"ArangoDB 配置": arango_config, "驗證結果": "一致"})
            else:
                config_info["verified"] = False
                config_info[
                    "note"
                ] = f"⚠️ ArangoDB 配置與預期模型不一致（NER: {actual_ner}, RE: {actual_re}, RT: {actual_rt}）"
                record_time(
                    "配置驗證失敗",
                    details={"ArangoDB 配置": arango_config, "預期模型": expected_model, "驗證結果": "不一致"},
                )
        else:
...
def monitor_processing(file_id: str, token: str, max_wait: int = 300) -> bool:
    ...
    while time.time() - start_time < max_wait:
        ...
        if overall_status == "completed":
            ...
            record_time("處理失敗", details={"錯誤信息": status.get("message", "Unknown error")})
...
def generate_markdown_report(md_file: Path):
    ...
    with open(md_file, "w", encoding="utf-8") as f:
        ...
        if arango_config:
            f.write("- **ArangoDB 配置**:\n")
            f.write(
                f"  - NER: {arango_config.get('ner_model_type', 'N/A')} - {arango_config.get('ner_model', 'N/A')}\n"
            )
            f.write(
                f"  - RE: {arango_config.get('re_model_type', 'N/A')} - {arango_config.get('re_model', 'N/A')}\n"
            )
            f.write(
                f"  - RT: {arango_config.get('rt_model_type', 'N/A')} - {arango_config.get('rt_model', 'N/A')}\n"
            )
...
def main():
    if len(sys.argv) < 3:
        ...
        print(
            "範例: python test_kg_model_comparison.py 149aee1a-89da-4b07-a83c-634fb29246e2 mistral-nemo:12b"
        )
        sys.exit(1)
