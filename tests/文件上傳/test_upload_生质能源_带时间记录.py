...
def login(username: str = "daniel@test.com", password: str = "test123") -> Optional[str]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if token:
                record_time("登錄成功", details={"耗時": f"{login_time:.3f}秒", "HTTP狀態": response.status_code})
                return token
            else:
                record_time("登錄失敗：無法獲取 access_token", details={"響應": result})
                return None
        else:
            record_time("登錄失敗", details={"HTTP狀態": response.status_code, "錯誤": response.text, "耗時": f"{login_time:.3f}秒"})
            return None
...
def upload_file(file_path: Path, token: str, task_id: Optional[str] = None) -> Optional[dict]:
    """上傳文件"""
    record_time("開始上傳文件", details={"文件名": file_path.name, "文件大小": f"{file_path.stat().st_size / 1024 / 1024:.2f} MB"})
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("success"):
                    ...
                    record_time("文件上傳失敗", details={"錯誤": result.get('message', 'Unknown error'), "耗時": f"{upload_time:.3f}秒"})
                    return None
            else:
                record_time("文件上傳失敗", details={"HTTP狀態": response.status_code, "錯誤": response.text, "耗時": f"{upload_time:.3f}秒"})
                return None
        except Exception as e:
    ...
    while True:
        ...
        if vectorization_status == "processing" and vectorization_start is None:
            ...
            if chunking_start:
                ...
                record_time("分塊階段完成", details={"耗時": f"{chunking_time:.2f}秒", "分塊數": chunking.get("chunk_count", 0)})
        ...
        if kg_extraction_status == "processing" and kg_extraction_start is None:
            ...
            if storage_start:
                ...
                record_time("存儲階段完成", details={"耗時": f"{storage_time:.2f}秒", "向量數": storage.get("vector_count", 0)})
