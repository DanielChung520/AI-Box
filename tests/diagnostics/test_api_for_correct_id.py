...
from database.redis.queue import get_redis_client
...
def test_api_logic():
    ...
    # Note: list_triples_by_file_id_with_acl is what the API uses
    # but for simplicity let's use list_triples_by_file_id
    triples_res = kg_builder.list_triples_by_file_id(fid)
    print(f"Triples Total: {triples_res.get('total')}")
    print(
        f"Triples Sample (first 2): {triples_res.get('triples')[:2] if triples_res.get('triples') else 'None'}"
    )
    ...
    print(f"Redis kg:chunk_state: {state_str}")
