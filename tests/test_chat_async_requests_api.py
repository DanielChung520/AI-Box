...
class _FakeContextManager:
    ...
    def record_message(
        ...
        *,
        ...
        _ = agent_name
        _ = metadata
        self._by_session.setdefault(session_id, []).append(_Msg(role=role, content=content))
        return True
...
def app_client(monkeypatch: pytest.MonkeyPatch) -> TestClient:
    ...
    monkeypatch.setattr(chat_router, "get_moe_manager", lambda: _FakeMoE(), raising=True)

    return TestClient(app)


def _poll_state(client: TestClient, request_id: str, *, timeout_s: float = 3.0) -> Dict[str, Any]:
    start = time.time()
    last: Dict[str, Any] = {}
...
def test_async_request_abort(app_client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    from api.routers import chat as chat_router
    ...
    monkeypatch.setattr(chat_router, "get_moe_manager", lambda: _SlowMoE(), raising=True)
