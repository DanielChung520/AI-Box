...
class OntologyManager:
    ...
    def merge_ontologies(self, domain_files: List[str], task_file: Optional[str] = None):
        """
        ...
        ONTOLOGY_RULES = {'entity_classes': [], 'relationship_types': [], 'owl_domain_range': {}}
                ...
                for prop in module['object_properties']:
                    ...
                    for domain_type in prop['domain']:
                        for range_type in prop['range']:
                            ONTOLOGY_RULES['owl_domain_range'][rel_name].append((domain_type, range_type))
        ...
        print(f"Ontology 合併完成。總實體數: {len(ONTOLOGY_RULES['entity_classes'])}，總關係數: {len(ONTOLOGY_RULES['relationship_types'])}")
    ...
    def generate_prompt(
        ...
        if not ontology_rules.get('entity_classes') and not ontology_rules.get('relationship_types'):
            raise RuntimeError(
...
if __name__ == '__main__':
    ...
    rules = manager.merge_ontologies(
        domain_files=['domain-enterprise.json'],
        task_file='major-manufacture.json'
    )
    ...
    try:
        ...
        if all(placeholder not in prompt for placeholder in [
            '[INJECT_ENTITY_LIST]', 
