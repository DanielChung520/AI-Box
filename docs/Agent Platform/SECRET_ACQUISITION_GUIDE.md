# Secret ID/Key 獲取方式指南

## 更新日期：2025-01-27

## 核心確認

✅ **外部 Agent 如果無法提供 Secret ID/Key，註冊程序會被阻止並退出。**

### 驗證點

1. **前端驗證**：未驗證 Secret 無法提交註冊表單
2. **後端驗證**：沒有 Secret ID 返回 HTTP 400 錯誤

---

## Secret ID/Key 獲取方式

### Phase 1：當前方式（測試/開發環境）

#### 方式 1：API 生成（用於測試）

**端點**：`POST /api/v1/agents/secrets/generate`

**使用方式**：

1. **通過 Swagger 文檔**：
   - 訪問 `http://localhost:8000/docs`
   - 找到 `POST /api/v1/agents/secrets/generate`
   - 執行請求，獲取 Secret ID/Key

2. **通過 curl 命令**：
   ```bash
   curl -X POST "http://localhost:8000/api/v1/agents/secrets/generate" \
     -H "Content-Type: application/json" \
     -d '{"organization": "Your Organization"}'
   ```

3. **通過前端 API 調用**（開發環境）：
   ```typescript
   const response = await generateSecret("Your Organization");
   // 返回 { secret_id, secret_key }
   ```

**注意事項**：
- ⚠️ 僅用於測試和開發環境
- ⚠️ Secret Key 只在生成時返回一次
- ⚠️ 生產環境應使用審批流程

#### 方式 2：聯繫管理員（當前生產環境）

- 直接聯繫 AI-Box 管理員
- 說明申請原因和使用場景
- 管理員手動生成並發送 Secret ID/Key

---

### Phase 2：完整申請流程（推薦實現）

#### 申請流程設計

```
外部 Agent 提供者
   ↓
提交申請（填寫組織信息、聯繫方式、Agent 描述）
   ↓
AI-Box 管理員審核
   ↓
審核通過 → 生成 Secret ID/Key
   ↓
通過郵件發送 Secret 給申請者
   ↓
申請者使用 Secret 註冊 Agent
```

#### 實施步驟

1. **創建申請表單頁面**
   - 組織名稱
   - 聯繫人信息
   - Agent 服務描述
   - 預期使用場景

2. **管理員審批界面**
   - 查看申請列表
   - 批准/拒絕申請
   - 自動生成並發送 Secret

3. **郵件通知系統**
   - 申請提交通知（給管理員）
   - Secret 簽發通知（給申請者）

---

## 建議實現方案

### 方案 A：快速申請 Modal（推薦 - 可立即實現）

在註冊頁面添加一個「快速申請」功能，點擊後：

1. 彈出簡單的表單 Modal
2. 填寫組織名稱和聯繫郵箱
3. 提交後調用 `generateSecret` API（開發環境）
   - 或提交申請（生產環境，等待審批）
4. 成功後自動填入 Secret ID/Key
5. 自動驗證並繼續註冊

**優點**：
- ✅ 快速實現
- ✅ 用戶體驗好
- ✅ 無需跳轉頁面

### 方案 B：獨立的申請頁面（Phase 2）

創建專門的 Secret 申請頁面，包含：

- 完整的申請表單
- 申請狀態查詢
- 歷史申請記錄

---

## 前端 UI 改進建議

### 當前狀態

在註冊頁面的 Secret 驗證區塊，有「點擊這裡申請」鏈接，目前顯示 alert 提示。

### 建議改進

將 alert 改為實際的申請功能：

```typescript
// 點擊「申請」時，彈出申請 Modal
const handleApplySecret = () => {
  setShowSecretApplicationModal(true);
};

// 申請成功後，自動填入
const handleApplicationSuccess = (secretId: string, secretKey: string) => {
  setSecretId(secretId);
  setSecretKey(secretKey);
  setShowSecretApplicationModal(false);
  // 可選：自動驗證
  // handleVerifySecret();
};
```

---

## 實施優先級

### 立即實現（Phase 1.5）

1. ✅ 在註冊頁面添加「快速生成 Secret」功能（開發環境）
2. ✅ 創建簡單的申請 Modal 組件

### 後續實現（Phase 2）

1. ✅ 完整的申請審批流程
2. ✅ 管理員審批界面
3. ✅ 郵件通知系統
4. ✅ 申請狀態追蹤

---

## 總結

### Secret 獲取方式總結

| 環境 | 獲取方式 | 狀態 |
|------|---------|------|
| 開發/測試 | API 生成 (`/agents/secrets/generate`) | ✅ 已實現 |
| 生產環境 | 聯繫管理員 | ✅ 當前可用 |
| 生產環境 | 申請審批流程 | 🔄 Phase 2 實現 |

### 註冊要求確認

- ✅ **外部 Agent 必須提供 Secret ID/Key**
- ✅ **沒有 Secret → 無法註冊，程序退出**
- ✅ **必須先驗證 Secret 才能提交註冊**
