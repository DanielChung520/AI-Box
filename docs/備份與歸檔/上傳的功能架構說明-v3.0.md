...
- 2026-01-20：v3.1 版本，更新向量存儲系統說明，明確 ChromaDB 配置（HTTP 模式）、移除閒置 SQLite 文件記錄
- 2026-01-20：添加 LLM 模型效能測試章節，基於 5 個 Ollama 模型的測試結果，推薦 `gpt-oss:120b-cloud` 作為首選模型
...
9. [向量化系統](#向量化系統)
10. [知識圖譜系統](#知識圖譜系統)
11. [LLM 模型效能測試](#llm-模型效能測試)
12. [文件上傳後顯示問題分析](#文件上傳後顯示問題分析)
13. [相關服務和依賴](#相關服務和依賴)
14. [文件存儲後端說明](#文件存儲後端說明)
15. [API 端點總結](#api-端點總結)
16. [數據流](#數據流)
17. [已知問題與改進計劃](#已知問題與改進計劃)
18. [ChromaDB 配置說明](#chromadb-配置說明)
...
**問題背景**：
在系統開發過程中，ChromaDB 的持久化路徑配置曾存在不一致，導致產生多個 SQLite 數據文件。

**歷史配置路徑**：

- `./data/chroma/chroma.sqlite3` - 已廢棄
- `./data/datasets/chromadb/chroma.sqlite3` - 已廢棄

**2026-01-20 處理結果**：

- ✅ 刪除所有閒置的 SQLite 文件
- ✅ 統一使用 Docker HTTP 模式（`chromadb:8001`）
- ✅ 更新配置文檔

**當前配置**：

- **模式**：HTTP（連接 Docker 容器 `chromadb:8001`）
- **持久化目錄**：不再使用本地 SQLite
- **配置來源**：`config/config.json` → `datastores.chromadb`
...
- **URL**: `POST /api/v1/files/v2/upload`
...
AI-Box 系統有兩個文件上傳路由，它們服務於不同的使用場景：

#### 2.5.1 路由總覽

| 特性 | `/api/v1/files/v2/upload` | `/api/v1/agent/files/upload` |
|------|--------------------------|------------------------------|
| **路由文件** | `api/routers/file_upload.py` | `api/routers/agent_files.py` |
| **觸發時機** | AI 聊天窗口上傳檔案 | Agent 編輯檔案時 |
| **處理流程** | 存儲 → 分塊 → 向量化 → KG 提取 | 只存儲 S3，確認提交後才處理 |
| **異步任務** | ✅ 自動觸發 RQ 任務 | ❌ 延遲處理 |
| **適用場景** | 用戶上傳參考文檔 | Agent 產出物臨時存儲 |

#### 2.5.2 AI 聊天窗口上傳 (`/files/v2/upload`)

**功能**: 用戶在 AI 聊天窗口上傳檔案，系統立即進行完整處理
...
創建 file_metadata 記錄
  ↓
提交 RQ 任務: process_file_chunking_and_vectorization_task
  ↓
異步處理:
  - 分塊 (ChunkProcessor)
...
```typescript
// ai-bot/src/lib/api.ts:628
const url = `${API_URL}/files/v2/upload`;
```

#### 2.5.3 Agent 文件上傳 (`/agent/files/upload`)

**功能**: Agent 編輯檔案時，先將檔案存儲到 S3，等檔案編輯確認提交後，才會單獨進行向量及圖譜處理
...
存儲到 SeaweedFS S3
  ↓
創建 file_metadata 記錄（status: uploaded）
  ↓
不觸發異步處理
  ↓
等待 Agent 編輯確認
...
- Agent 編輯模式下，臨時保存編輯中的檔案
- 檔案內容可能會被多次修改，不需要每次都重新處理
- 只有確認提交後才進行向量化和知識圖譜提取

#### 2.5.4 選擇指南

| 場景 | 使用路由 |
|------|----------|
| 用戶在聊天窗口上傳參考文檔 | `/files/v2/upload` |
| 用戶上傳文檔進行 RAG 檢索 | `/files/v2/upload` |
| Agent 編輯模式臨時保存 | `/agent/files/upload` |
| Agent 產出物上傳 | `/agent/files/upload` |
...
- **路由**: `@router.post("/v2/upload")`
- **完整路徑**: `/api/v1/files/v2/upload`
- **位置**: 第 2047-2564 行
...
- **上傳文件**：`POST /api/v1/files/v2/upload`
...
**存儲位置**：ChromaDB（HTTP 模式，Docker 容器）

**配置優先級**：

1. **環境變量**：`CHROMADB_HOST`, `CHROMADB_PORT`, `CHROMADB_MODE`
2. **配置文件**：`config/config.json` → `datastores.chromadb`
3. **向後兼容**：`config/config.json` → `chromadb`

**當前配置示例**（`config/config.json`）：
...
- **Host**：`localhost`（本地開發）或 Docker 容器名稱
- **Port**：`8001`
- **Mode**：`http`（生產環境）或 `persistent`（開發測試，可選）

**歷史配置說明**：

- 早期版本使用 `persistent` 模式，數據存儲在本地 SQLite 文件
- 為支援 Kubernetes 無狀態擴展，現改用 HTTP 模式連接 Docker 容器
- 詳見 [ChromaDB 配置說明](#chromadb-配置說明)
...
**測試目標**: 比較 5 個 Ollama 模型在知識圖譜提取任務上的效能，選擇最佳模型用於文件上傳流程中的 LLM 調用（文件摘要生成、Ontology 選擇、三元組提取）。

**測試方法**: 使用標準測試文本（1935 字元），調用各模型進行知識圖譜提取，測量提取時間、Entity 數量、Relation 數量。

### 測試模型列表

| 模型 | 說明 |
|------|------|
| `gpt-oss:120b-cloud` | 120B 參數開源模型（Ollama） |
| `glm-4.7:cloud` | ChatGLM 4.7B 模型（Ollama Cloud） |
| `glm-4.6:cloud` | ChatGLM 4.6B 模型（Ollama Cloud） |
| `qwen3-next:latest` | 通義千問 Next 模型 |
| `mistral-nemo:12b` | Mistral Nemo 12B 模型 |
...
| qwen3-next:latest | Timeout | 0 | 0 | 0% | 4.0 |

### 詳細分析

#### 🥇 第一名：gpt-oss:120b-cloud

**優勢**:

- ✅ **速度最快**（29.74s），比第二名快 48%
...
```json
{
  ...
  "genai": {
    "policy": {
      "allowed_providers": ["ollama", "chatgpt", "gemini", "qwen", "grok"],
      "default_fallback": {
        "provider": "ollama",
      ...
      "models": [
        {
          "provider": "ollama",
          "model_id": "gpt-oss:120b-cloud",
          "display_name": "GPT-OSS 120B（首選）"
        },
        {
          "provider": "ollama",
          "model_id": "glm-4.6:cloud",
          "display_name": "GLM 4.6 Cloud（備選-最大提取量）"
        }
      ]
    }
  }
...
| 場景 | 推薦模型 | 原因 |
|------|----------|------|
| 標準文件處理 | `gpt-oss:120b-cloud` | 速度快，成功率高 |
| 複雜技術文檔 | `glm-4.6:cloud` | 提取量最大 |
| 批量處理 | `gpt-oss:120b-cloud` | 速度快，適合批量 |
| 備用模型 | `mistral-nemo:12b` | 穩定但速度較慢 |

### 測試腳本位置

- **腳本**: `scripts/llm_performance_comparison.py`
- **使用方法**: `python scripts/llm_performance_comparison.py`
- **選項**: `--models` 指定模型，`--iterations` 指定迭代次數

### 更新記錄

- **2026-01-20**: 新增 LLM 效能測試章節，基於 5 個 Ollama 模型的測試結果
...
- **2026-01-20**: 明確兩個文件上傳路由的區別：
  - `/api/v1/files/v2/upload`: AI 聊天窗口上傳，觸發完整處理管道（存儲 → 分塊 → 向量化 → KG 提取）
  - `/api/v1/agent/files/upload`: Agent 編輯檔案臨時存儲，確認提交後才處理
  - 更新前端 API 調用從 `/files/upload` 改為 `/files/v2/upload`
  - 添加路由區別說明章節
...
ChromaDB 是 AI-Box 系統的向量數據庫，用於存儲文件向量化後的 embedding 數據，支援語義搜尋功能。
...
| 路徑 | 狀態 | 說明 |
|------|------|------|
| `./data/chroma/chroma.sqlite3` | ⚠️ 已刪除 (2026-01-20) | `database/chromadb/client.py` 預設路徑 |
| `./data/datasets/chromadb/chroma.sqlite3` | ⚠️ 已刪除 (2026-01-20) | `vector_store_service.py` 預設路徑 |

**根本原因**：

1. `database/chromadb/client.py` 預設路徑：`./data/chroma_data`
2. `services/api/services/vector_store_service.py` 預設路徑：`./data/datasets/chromadb`
3. 兩個模組使用不同的預設路徑，導致數據分散
...
| 參數 | 預設值 | 說明 |
|------|--------|------|
| `host` | `localhost` | Docker 環境下可使用容器名稱 |
| `port` | `8001` | ChromaDB HTTP 服務端口 |
| `mode` | `http` | 連接模式：`http` 或 `persistent` |

### Docker 部署

系統使用 Docker 運行 ChromaDB：
...
- 使用 Kubernetes 部署時，配置 PersistentVolumeClaim
- 或使用托管向量數據庫服務
...
1. **連接失敗**
   - 檢查 Docker 容器是否運行：`docker ps | grep chroma`
   - 檢查端口是否正確：`config/config.json` 中的 `http_port`

2. **數據丟失**
   - HTTP 模式下數據由容器管理
   - 重啟容器會清空數據（除非配置 volume 掛載）

### 相關文件

- 配置模板：`config/config.example.json`
- ChromaDB 客戶端：`database/chromadb/client.py`
- 向量存儲服務：`services/api/services/vector_store_service.py`

---

## 📋 測試計劃與記錄（2026-01-20 新增）

### SystemDoc 文件上傳測試計劃 v3.1
...
- **處理階段**: 文件上傳、文件分塊、向量化、知識圖譜提取

**相關文檔**：

- [SystemDoc文件上传测试计划-v3.1](./SystemDoc文件上传测试计划-v3.1.md)
- [SystemDoc文件上传测试记录-v3.1](./SystemDoc文件上传测试记录-v3.1.md)

### 第一階段：準備工作（2026-01-20 執行）

**執行結果**：

- ✅ 環境確認完成（所有服務運行正常）
- ✅ 數據清理完成（SeaWeedFS、ArangoDB、ChromaDB）
...
| ArangoDB entities | 1251  個 |
|個 | 0 ArangoDB relations | 937 個 | 0 個 |
