...
- 文件內容預覽
- 向量數據查看（如果已生成）- **使用 Qdrant 向量數據庫存儲**
- 知識圖譜查看（如果已生成）
- 文件下載功能

**向量數據庫**：系統使用 **Qdrant** 作為向量數據庫（已於 2026-01-20 從 ChromaDB 遷移），用於存儲和檢索文件的向量化數據。每個文件對應一個 Qdrant Collection，命名格式為 `file_{file_id}`。
...
**功能**: 獲取文件的向量數據（從 Qdrant 向量數據庫查詢）

**向量數據庫**: Qdrant（已於 2026-01-20 從 ChromaDB 遷移）

**Collection 命名策略**: `file_{file_id}`（每個文件一個 Collection）

**參數**:

- `file_id` (path): 文件 ID
- `limit` (query): 返回數量限制（默認 100，最大 1000）
...
```json
{
  ...
  "data": {
    ...
    "stats": {
      "vector_count": 100,
      "dimension": 1536,
      "collection_name": "file_cc3d7aee-b5b3-4e11-9458-784575c1dba6"
    }
  }
}
```
...
- **id**: 向量 ID（格式：`chunk_{index}`）
- **vector**: 向量數據（浮點數數組，維度通常為 1536）
- **payload**: 元數據
  - `file_id`: 文件 ID
  - `chunk_index`: 文本塊索引
  - `chunk_text`: 文本塊內容
...
**後端服務**: `QdrantVectorStoreService` (`services/api/services/qdrant_vector_store_service.py`)
...
- **Qdrant**: 向量數據庫（已於 2026-01-20 從 ChromaDB 遷移）
  - **服務**: `QdrantVectorStoreService` (`services/api/services/qdrant_vector_store_service.py`)
  - **端口**: REST API 6333，gRPC 6334
  - **Collection 命名**: `file_{file_id}`（每個文件一個 Collection）
  - **向量維度**: 通常為 1536（取決於 Embedding 模型）

**注意**: 前端不直接訪問 Qdrant，而是通過 REST API (`GET /api/v1/files/{file_id}/vectors`) 獲取向量數據。
...
**文檔版本**: 1.1
**最後更新**: 2026-01-21 11:52 UTC+8
**維護者**: Daniel Chung

**更新記錄**:

- 2026-01-21: 更新向量數據庫說明，明確使用 Qdrant（從 ChromaDB 遷移）
