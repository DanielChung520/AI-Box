...
class HybridRAGService:
    ...
    def retrieve(
        ...
        try:
            ...
            self.logger.info(f"Hybrid RAG retrieval completed: query={ query[:50] }, count={ len(formatted_results }",
            ...
            self.logger.error(f"Failed to perform hybrid RAG retrieval: error={ str(e )}")
            return []
    ...
    def update_strategy(self, strategy: RetrievalStrategy) -> None:
        """更新檢索策略"""
        self.strategy = strategy
        self.logger.info(f"Updated retrieval strategy: strategy={ strategy.value}")
    ...
    def update_weights(self, vector_weight: float, graph_weight: float) -> None:
        ...
        if total > 0:
            ...
            self.logger.info(f"Updated retrieval weights: vector_weight={ self.vector_weight }, graph_weight={ self.graph_weight}")
