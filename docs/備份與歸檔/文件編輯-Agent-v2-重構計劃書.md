...
- 2026-01-21：更新文件路徑格式為 S3 URI（SeaWeedFS）；更新 VectorDB 為 Qdrant；添加存儲後端說明章節
...
4. **任務工作區**：所有新文件創建在任務工作區（`s3://bucket-ai-box-assets/tasks/{task_id}/workspace/`）下
...
3. **整合任務工作區**：新文件必須創建在任務工作區（`s3://bucket-ai-box-assets/tasks/{task_id}/workspace/`）下
...
1. **任務工作區查詢**（2 天）
   ...
   - 實現任務工作區存在性檢查
   - 實現任務工作區自動創建（如果不存在）
    - **重要**：新文件必須創建在 `s3://bucket-ai-box-assets/tasks/{task_id}/workspace/` 目錄下

2. **文件創建邏輯**（2 天）
   - 實現新文件創建（通過 `FileMetadataService`）
    - 實現文件路徑生成（`s3://bucket-ai-box-assets/tasks/{task_id}/workspace/{file_id}.{ext}`）
   - 實現文件存儲（通過 `Storage`）
...
- ✅ 新文件必須創建在任務工作區下（`s3://bucket-ai-box-assets/tasks/{task_id}/workspace/`）
...
| **向量數據庫** | Qdrant | 高性能向量檢索，取代 ChromaDB |

### 5.2 依賴服務

| 服務 | 用途 | 位置 |
|------|------|------|
| **S3FileStorage** | 文件存儲（SeaWeedFS S3） | `storage/s3_storage.py` |
...
| **QdrantVectorStoreService** | 向量存儲與檢索 | `services/api/services/qdrant_vector_store_service.py` |
| **ArangoDBClient** | 數據庫連接 | `database/arangodb.py` |
| **LLM Client** | LLM 調用 | 現有 LLM 基礎設施 |

### 5.3 存儲後端說明

#### 5.3.1 文件存儲（SeaWeedFS S3）

文件使用 SeaWeedFS S3 API 存儲，採用以下結構：
...
- S3 端口：8333
- 服務類型：`SeaweedFSService.AI_BOX`

#### 5.3.2 向量存儲（Qdrant）

向量檢索使用 Qdrant 向量數據庫：

**Collection 命名**：`file_{file_id}`
