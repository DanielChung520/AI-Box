...
1. 登入 AI-Box 系統
2. 在文件列表中選擇要編輯的 Markdown 文件
3. 點擊「使用 IEE 編輯器打開」按鈕
...
2. 選擇文件類型：
   - **Markdown 文件** (.md)：直接打開
   - **PDF 文件** (.pdf)：自動轉換為 Markdown
   - **Word 文件** (.docx)：自動轉換為 Markdown
3. 等待文件處理完成
...
| 快捷鍵 | 功能 |
|--------|------|
| `Ctrl+Enter` / `Cmd+Enter` | 發送 AI 指令 |
| `Ctrl+K` / `Cmd+K` | 快速 AI 編輯（選中文字後） |
| `Esc` | 取消當前 AI 建議 |
...
| 快捷鍵 | 功能 |
|--------|------|
| `Ctrl+Shift+R` / `Cmd+Shift+R` | 進入審查模式 |
| `Ctrl+.]` | 下一個變更 |
| `Ctrl+,[` | 上一個變更 |
| `Ctrl+Enter` | 接受當前修改 |
| `Ctrl+Esc` | 拒絕當前修改 |
