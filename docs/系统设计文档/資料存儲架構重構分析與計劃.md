...
- 2025-12-29：補充階段一任務 1.1.1 和 1.1.2 的實際部署步驟說明（Kubernetes 和 Docker Compose 部署方式）
...
- ⚠️ **部署步驟**：詳細的部署步驟已添加到任務 1.1.1 和 1.1.2 中，包括 Kubernetes 和 Docker Compose 兩種部署方式。
...
1. **Kubernetes 環境部署**：
   ...
   ```bash
   # 1. 部署 AI-Box SeaweedFS 服務
   kubectl apply -f k8s/seaweedfs-ai-box/

   # 2. 檢查部署狀態
   kubectl get pods -n ai-box | grep seaweedfs-ai-box
   kubectl get svc -n ai-box | grep seaweedfs-ai-box

   # 3. 等待所有 Pod 就緒
   kubectl wait --for=condition=ready pod -l app=seaweedfs-ai-box -n ai-box --timeout=300s
   ```

2. **本地開發環境部署（Docker Compose）**：

   ```bash
   # 1. 創建 docker-compose.seaweedfs.yml（如果不存在）
   # 參考：docs/系统设计文档/开发环境设置指南.md
   ...
   curl http://localhost:8333  # 檢查 S3 API
   curl http://localhost:8888  # 檢查 Filer API
   ```
...
3. **部署後驗證**：
   ...
   ```bash
   # 1. 檢查服務健康狀態
   kubectl get pods -n ai-box -l app=seaweedfs-ai-box

   # 2. 檢查服務端點
   kubectl get svc -n ai-box | grep seaweedfs-ai-box

   # 3. 測試 S3 API 連接
   python scripts/migration/create_seaweedfs_buckets.py --service ai_box --dry-run
   ```
...
1. **Kubernetes 環境部署**：
   ...
   ```bash
   # 1. 部署 DataLake SeaweedFS 服務
   kubectl apply -f k8s/seaweedfs-datalake/

   # 2. 檢查部署狀態
   kubectl get pods -n datalake | grep seaweedfs-datalake
   kubectl get svc -n datalake | grep seaweedfs-datalake

   # 3. 等待所有 Pod 就緒
   kubectl wait --for=condition=ready pod -l app=seaweedfs-datalake -n datalake --timeout=300s
   ```

2. **本地開發環境部署（Docker Compose）**：

   ```bash
   # 注意：DataLake SeaweedFS 服務在本地開發環境中可選
   # 如果需要，可以創建獨立的 docker-compose.seaweedfs-datalake.yml
   # 或使用不同的端口避免與 AI-Box 服務衝突

   # 1. 啟動 DataLake SeaweedFS 服務（使用不同端口）
   # 例如：S3 API 使用 8334，Filer API 使用 8889

   # 2. 檢查服務狀態
   docker ps | grep seaweedfs-datalake
   curl http://localhost:8334  # 檢查 S3 API
   curl http://localhost:8889  # 檢查 Filer API
   ```
...
3. **部署後驗證**：
   ...
   ```bash
   # 1. 檢查服務健康狀態
   kubectl get pods -n datalake -l app=seaweedfs-datalake

   # 2. 檢查服務端點
   kubectl get svc -n datalake | grep seaweedfs-datalake

   # 3. 測試 S3 API 連接
   python scripts/migration/create_seaweedfs_buckets.py --service datalake --dry-run
   ```
