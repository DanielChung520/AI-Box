...
| 2026-01-21 | v2.1 | 添加 DocumentContext 數據模型；添加存儲服務整合說明（S3FileStorage、QdrantVectorStoreService） |
...
### DocumentContext（文檔上下文）

**用途**：定義文件編輯的上下文，包含文件位置和用戶信息

**位置**：`agents/builtin/document_editing_v2/models.py`

```python
class DocumentContext(BaseModel):
    doc_id: str                          # 文檔唯一標識
    version_id: Optional[str]            # 版本 ID
    file_path: str                       # 文件路徑（S3 URI）
    task_id: str                         # 任務 ID
    user_id: str                         # 用戶 ID
    tenant_id: Optional[str]             # 租戶 ID
    storage_backend: Optional[str]       # 存儲後端（seaweedfs）
```

**文件路徑格式**：

- **S3 URI**：`s3://bucket-ai-box-assets/tasks/{task_id}/workspace/{file_id}.{ext}`
- **示例**：`s3://bucket-ai-box-assets/tasks/task-123/workspace/cc3d7aee-b5b3-4e11-9458-784575c1dba6.md`

**存儲後端**：

- `seaweedfs`：使用 SeaWeedFS S3 API 存儲
- `local`：本地文件系統（開發環境）
...
**位置**：`storage/s3_storage.py`

**功能**：

- `read_file()`：從 SeaWeedFS 讀取文件內容
- `write_file()`：將文件寫入 SeaWeedFS
- `delete_file()`：從 SeaWeedFS 刪除文件
...
content = storage.read_file(
    ...
    key="tasks/task-123/workspace/file-id.md"
)
```
