...
| **8005**       | Datalake Frontend API             | Data-Agent 體系       | 數據湖儀表板後端 API（DataLakeClient） |
...
| **8503**       | Datalake Frontend (Vite)          | Data-Agent 體系       | 數據湖儀表板前端開發服務               |
...
| **~~8502~~** | ~~Tiptop Data-Agent Dashboard~~ | ~~Data-Agent 體系~~ | ~~Streamlit 儀表板（已經棄用)~~      |
...
### 4.3 Datalake Frontend API（數據湖儀表板後端）

| 端口           | 服務                  | 配置來源                                   | 環境變數     |
| -------------- | --------------------- | ------------------------------------------ | ------------ |
| **8005** | Datalake Frontend API | `datalake-system/frontend/api_server.py` | `API_PORT` |

**啟動**：`python datalake-system/frontend/api_server.py`（預設端口 8005）

**端點**：

- `http://localhost:8005/health` - 健康檢查
- `http://localhost:8005/api/v1/datalake/*` - 數據湖 API（庫存、交易等）
- 依賴 Data Agent（`http://localhost:8004`）執行查詢

**說明**：為數據湖 React 前端（8503）提供後端 API，封裝 DataLakeClient 與 Data Agent 調用。

### 4.4 Datalake Frontend（Vite 開發服務）

| 端口           | 服務                     | 配置來源                                                      | 說明                                           |
| -------------- | ------------------------ | ------------------------------------------------------------- | ---------------------------------------------- |
| **8503** | Datalake Frontend (Vite) | `datalake-system/frontend/package.json`、`vite.config.ts` | `npm run dev` → `vite --host --port 8503` |

**啟動**：

```bash
cd datalake-system/frontend
npm run dev
```

**說明**：數據湖儀表板 React 前端（NLP 查詢、儀表板 UI）。前端請求會代理至 Frontend API（8005）與 MM-Agent（8003）。

### 4.5 Tiptop Data-Agent Dashboard（Streamlit 儀表板）

| 端口           | 服務             | 配置來源                                                        | 說明                          |
| -------------- | ---------------- | --------------------------------------------------------------- | ----------------------------- |
| **8502** | Tiptop Dashboard | `datalake-system/dashboard/start.sh`、`dashboard/config.py` | `DASHBOARD_PORT`，預設 8502 |
...
**說明**：Tiptop 數據湖 Streamlit 儀表板，展現自然語言查詢與數據分析能力。依賴 Datalake SeaweedFS 與 Data Agent。
...
├── MM-Agent / Warehouse Manager Agent (8003) ← 依賴 Data Agent (8004)
├── Datalake Frontend API (8005) ← 依賴 Data Agent (8004)，供前端 8503 調用
├── Datalake Frontend (8503) ← Vite 開發服務，調用 8005、8003
└── Tiptop Data-Agent Dashboard (8502) ← Streamlit，依賴 Datalake SeaweedFS 與 Data Agent
```
...
```bash
# 檢查常用端口
for port in 3000 6333 6379 8000 8002 8003 8004 8005 8333 8334 8529 8502 8503 8888 8889 9181 9333 9334; do
