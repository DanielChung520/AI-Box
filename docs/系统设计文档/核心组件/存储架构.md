# 存储架构文档

**创建日期**: 2025-12-29
**创建人**: Daniel Chung
**最后修改日期**: 2026-01-13

**更新记录**：

- 2025-12-29：文档已创建，包含完整的 SeaweedFS 双服务部署架构说明
- 2026-01-13：添加 S3 API 启用配置说明，更新端口配置信息，添加 Docker Compose 配置说明

---

## 📋 概述

本文档描述 AI-Box 系统的存储架构，包括 SeaweedFS 双服务部署架构、S3 存储实现和配置说明。

---

## 🏗️ 存储架构设计

### 双服务架构

AI-Box 系统使用 **SeaweedFS 双服务部署架构**，分别为 AI-Box 和 DataLake 项目提供独立的存储服务：

1. **AI-Box SeaweedFS 服务**：存放 AI-Box 项目内的非结构化数据
2. **DataLake SeaweedFS 服务**：存放 DataLake 项目的文件备份数据

**架构优势**：

- ✅ **职责分离**：AI-Box 和 DataLake 各自管理自己的存储
- ✅ **独立扩展**：两个服务可以根据各自需求独立扩展
- ✅ **数据隔离**：避免两个项目之间的数据混杂
- ✅ **灵活部署**：可以根据实际需求选择不同的部署策略

### 存储后端支持

系统支持两种存储后端：

1. **本地文件系统** (`LocalFileStorage`)：用于开发和测试环境
2. **S3/SeaweedFS** (`S3FileStorage`)：用于生产环境，支持 Kubernetes 无状态扩展

---

## 🔧 SeaweedFS 服务配置

### AI-Box SeaweedFS 服务

**服务名称**：`seaweedfs-ai-box`

**主要存储内容**：

- **治理相关日志**：
  - 审计日志（Append-Only）
  - 系统日志（Append-Only）
- **版本历史记录**：配置和 Ontology 的历史版本（Append-Only）
- **变更提案**：变更提案记录（Append-Only）
- **DataLake 元数据**：
  - Data Agent 保存的 DataLake dictionary 定义
  - Data Agent 保存的 DataLake schema 定义
- **AI-Box 项目其他非结构化数据**

**Buckets**：

- `bucket-governance-logs`：治理相关日志
- `bucket-version-history`：版本历史记录
- `bucket-change-proposals`：变更提案记录
- `bucket-datalake-dictionary`：DataLake dictionary 定义
- `bucket-datalake-schema`：DataLake schema 定义
- `bucket-ai-box-assets`：AI-Box 项目其他非结构化数据

**端口配置**：

- **Master API**：9333（容器内）/ 9333（主机）
- **Filer API**：8888（容器内）/ 8888（主机）
- **S3 API**：8333（容器内）/ 8333（主机）

**环境变量配置**：

```bash
AI_BOX_SEAWEEDFS_S3_ENDPOINT=http://localhost:8333
AI_BOX_SEAWEEDFS_S3_ACCESS_KEY=admin
AI_BOX_SEAWEEDFS_S3_SECRET_KEY=admin123
AI_BOX_SEAWEEDFS_USE_SSL=false
AI_BOX_SEAWEEDFS_FILER_ENDPOINT=http://localhost:8888
```

**Docker Compose 配置**：

Filer 节点需要启用 S3 API，配置如下：

```yaml
seaweedfs-filer:
  command: "filer -master=seaweedfs-master:9333 -s3 -s3.port=8333 -s3.config=/etc/seaweedfs/s3.json"
  volumes:
    - seaweedfs-ai-box-s3-config:/etc/seaweedfs
```

**S3 配置文件**（`s3.json`）：

```json
{
  "identities": [
    {
      "name": "admin",
      "credentials": [
        {
          "accessKey": "admin",
          "secretKey": "admin123"
        }
      ],
      "actions": [
        "Admin",
        "Read",
        "Write"
      ]
    }
  ]
}
```

### DataLake SeaweedFS 服务

**服务名称**：`seaweedfs-datalake`

**主要存储内容**：

- 文件备份数据
- DataLake 项目相关的存储需求

**Buckets**：

- `bucket-file-backups`：文件备份数据
- `bucket-datalake-assets`：DataLake 项目相关存储需求

**端口配置**：

- **Master API**：9333（容器内）/ 9334（主机）
- **Filer API**：8888（容器内）/ 8889（主机）
- **S3 API**：8333（容器内）/ 8334（主机）

**环境变量配置**：

```bash
DATALAKE_SEAWEEDFS_S3_ENDPOINT=http://localhost:8334
DATALAKE_SEAWEEDFS_S3_ACCESS_KEY=admin
DATALAKE_SEAWEEDFS_S3_SECRET_KEY=admin123
DATALAKE_SEAWEEDFS_USE_SSL=false
DATALAKE_SEAWEEDFS_FILER_ENDPOINT=http://localhost:8889
DATALAKE_SEAWEEDFS_MASTER_HOST=localhost
DATALAKE_SEAWEEDFS_MASTER_PORT=9334
```

**Docker Compose 配置**：

Filer 节点需要启用 S3 API，配置如下：

```yaml
seaweedfs-datalake-filer:
  command: "filer -master=seaweedfs-datalake-master:9333 -s3 -s3.port=8333 -s3.config=/etc/seaweedfs/s3.json"
  ports:
    - "8889:8888" # Filer API
    - "8334:8333" # S3 API
  volumes:
    - seaweedfs-datalake-s3-config:/etc/seaweedfs
```

**S3 配置文件**（`s3.json`）：

```json
{
  "identities": [
    {
      "name": "admin",
      "credentials": [
        {
          "accessKey": "admin",
          "secretKey": "admin123"
        }
      ],
      "actions": [
        "Admin",
        "Read",
        "Write"
      ]
    }
  ]
}
```

**重要提示**：

⚠️ **S3 API 必须显式启用**：SeaweedFS Filer 默认不启用 S3 API，必须在启动命令中添加 `-s3` 参数，否则 S3 API 端口虽然映射了，但服务不会响应请求。

---

## 💻 代码实现

### S3FileStorage 类

`S3FileStorage` 类实现了 `FileStorage` 抽象接口，使用 `boto3` 连接 SeaweedFS S3 API。

**主要功能**：

- 文件上传、下载、删除
- Bucket 自动创建
- 支持 task_id 组织文件路径
- 支持双服务选择（AI-Box vs DataLake）
- 错误处理和日志记录

**使用示例**：

```python
from storage.s3_storage import S3FileStorage, SeaweedFSService

# 创建 AI-Box 服务的存储实例
storage = S3FileStorage(
    endpoint="http://seaweedfs-ai-box-filer:8333",
    access_key="your-access-key",
    secret_key="your-secret-key",
    service_type=SeaweedFSService.AI_BOX,
)

# 保存文件
file_id, s3_uri = storage.save_file(
    file_content=b"file content",
    filename="test.txt",
)

# 读取文件
content = storage.read_file(file_id=file_id)
```

### 配置创建存储实例

使用 `create_storage_from_config` 函数从配置创建存储实例：

```python
from storage.file_storage import create_storage_from_config

# 从配置创建（支持环境变量和配置文件）
config = {
    "storage_backend": "s3",
    "service_type": "ai_box",  # 或 "datalake"
}

storage = create_storage_from_config(config, service_type="ai_box")
```

**配置优先级**：

1. 函数参数（`service_type`）
2. 配置文件（`config["service_type"]`）
3. 环境变量（根据服务类型选择对应的环境变量）
4. 默认值（AI-Box 服务）

---

## 🚀 部署指南

### Kubernetes 部署

SeaweedFS 服务通过 Kubernetes 部署，包括以下组件：

1. **Master 节点**：管理元数据和 Volume 节点
2. **Volume 节点**：存储实际数据
3. **Filer 节点**：提供文件系统接口和 S3 API

**部署文件位置**：

- `k8s/seaweedfs-ai-box/`：AI-Box 服务部署配置
- `k8s/seaweedfs-datalake/`：DataLake 服务部署配置

### Buckets 创建

使用 `scripts/migration/create_seaweedfs_buckets.py` 脚本创建所有必要的 Buckets：

```bash
# 创建所有 Buckets（AI-Box 和 DataLake）
python scripts/migration/create_seaweedfs_buckets.py --service all

# 只创建 AI-Box 服务的 Buckets
python scripts/migration/create_seaweedfs_buckets.py --service ai_box

# 只创建 DataLake 服务的 Buckets
python scripts/migration/create_seaweedfs_buckets.py --service datalake

# 乾運行模式（不實際創建）
python scripts/migration/create_seaweedfs_buckets.py --service all --dry-run
```

---

## 📝 开发指南

### 环境配置

1. **复制环境变量示例文件**：

   ```bash
   cp env.example .env
   ```

2. **配置 SeaweedFS 连接信息**：
   编辑 `.env` 文件，设置 AI-Box 和 DataLake 服务的 SeaweedFS 配置。

3. **验证配置**：
   确保所有必要的环境变量都已设置。

### 代码使用

1. **导入存储模块**：

   ```python
   from storage import S3FileStorage, SeaweedFSService
   from storage.file_storage import create_storage_from_config
   ```

2. **创建存储实例**：

   ```python
   # 方式一：直接创建
   storage = S3FileStorage(
       endpoint=os.getenv("AI_BOX_SEAWEEDFS_S3_ENDPOINT"),
       access_key=os.getenv("AI_BOX_SEAWEEDFS_S3_ACCESS_KEY"),
       secret_key=os.getenv("AI_BOX_SEAWEEDFS_S3_SECRET_KEY"),
       service_type=SeaweedFSService.AI_BOX,
   )

   # 方式二：从配置创建
   config = get_config_section("file_upload", default={}) or {}
   storage = create_storage_from_config(config, service_type="ai_box")
   ```

3. **使用存储接口**：

   ```python
   # 保存文件
   file_id, s3_uri = storage.save_file(
       file_content=content,
       filename="example.txt",
       task_id="task-123",  # 可选
   )

   # 读取文件
   content = storage.read_file(file_id=file_id)

   # 删除文件
   success = storage.delete_file(file_id=file_id)

   # 检查文件是否存在
   exists = storage.file_exists(file_id=file_id)
   ```

---

## 🔍 测试

### 单元测试

运行存储模块的单元测试：

```bash
# 运行所有存储测试
pytest tests/storage/

# 运行特定测试文件
pytest tests/storage/test_s3_storage.py

# 运行特定测试用例
pytest tests/storage/test_s3_storage.py::TestS3FileStorage::test_save_file
```

### 集成测试

在集成测试中，确保 SeaweedFS 服务可用，并配置正确的环境变量。

---

## 📚 相关文档

- [资料存储架构重构分析与计划](../資料存儲架構重構分析與計劃.md)
- [资料架构建议报告](../資料架构建议报告.md)
- [系统设计文档主README](../README.md)

---

**最后更新日期**: 2026-01-13
