...
| 向量化 (Vectorization) | 分塊結果 (`chunks`) | 需要 `chunks` 中的 `text` 字段 |
...
**方法**：`VectorStoreService.delete_vectors_by_file_id()`
...
- [向量化與圖譜提取依賴關係分析](./向量化與圖譜提取依賴關係分析.md) - 問題分析文檔（待創建）
