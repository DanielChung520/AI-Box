...
- 基於 5 個 Ollama 模型的效能測試結果
- 新增推薦模型配置（`gpt-oss:120b-cloud` 作為首選）
...
| 模型 | 耗時 | Entities | Relations | 成功率 | 綜合得分 |
|------|------|----------|-----------|--------|----------|
| **gpt-oss:120b-cloud** | 29.74s | 77 | 77 | 100% | **89.2** |
| **glm-4.6:cloud** | 57.02s | **95** | **93** | 100% | **80.9** |
| glm-4.7:cloud | 106.22s | 65 | 63 | 100% | 52.0 |
| mistral-nemo:12b | 93.80s | 55 | 53 | 100% | 47.1 |
| qwen3-next:latest | Timeout | 0 | 0 | 0% | 不推薦 |

### 模型選擇建議

| 場景 | 推薦模型 | 原因 |
|------|----------|------|
| **標準文件處理** | `gpt-oss:120b-cloud` | 速度快（29.74s），成功率高 |
| **複雜技術文檔** | `glm-4.6:cloud` | 提取量最大（95 entities） |
| **批量處理** | `gpt-oss:120b-cloud` | 速度快，適合批量 |
| **備用模型** | `mistral-nemo:12b` | 穩定但速度較慢 |
...
### Ollama 模型 - gpt-oss:120b-cloud（首選）
...
- ✅ 首選模型，速度最快（29.74s）
- ✅ 提取品質穩定（77 entities, 77 relations）
- ✅ 綜合得分最高（89.2 分）

### Ollama 模型 - glm-4.6:cloud（備選-最大提取量）
...
- ✅ 提取量最大（95 entities, 93 relations）
- ⚠️ 速度中等（57.02s）
- 適合需要更多實體和關係的深度分析

### Ollama 模型 - mistral-nemo:12b（備選）
...
5. ✅ **模型选择**: 优先使用 `gpt-oss:120b-cloud`（速度最快）
6. ✅ **备选方案**: `glm-4.6:cloud`（提取量最大）

**模型推荐（2026-01-20 更新）**:
| 優先級 | 模型 | 特點 |
|--------|------|------|
| 🥇 首選 | `gpt-oss:120b-cloud` | 速度最快（29.74s），成功率 100% |
| 🥈 備選 | `glm-4.6:cloud` | 提取量最大（95 entities） |
| ⚠️ 謹慎 | `mistral-nemo:12b` | 速度較慢（93.80s） |
| ❌ 不推薦 | `qwen3-next:latest` | 超時失敗 |

**实施建议**:

- ✅ 立即采用推荐参数
- ✅ 在服务层实现配置化
- ✅ 使用 `gpt-oss:120b-cloud` 作为首选模型
- ✅ 通过 A/B 测试优化参数
- ✅ 根据实际效果调整模型选择
