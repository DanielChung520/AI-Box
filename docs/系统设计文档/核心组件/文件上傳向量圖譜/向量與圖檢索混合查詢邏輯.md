...
本文檔詳細說明基於 `HybridRAGService._graph_retrieval()` 方法的向量與圖檢索混合查詢邏輯，描述如何結合向量檢索（ChromaDB）和圖譜檢索（ArangoDB）實現 Hybrid RAG（混合檢索增強生成）系統。
...
- ⚠️ **注意**：當前實現依賴 AAM 系統，不是直接使用 ChromaDB
...
**向量檢索實現**:
- **文檔**: 描述使用 ChromaDB 進行向量檢索
- **實現**: 實際使用 AAM 系統的 `RealtimeRetrievalService`，不是直接使用 ChromaDB
- **差異**: 實現依賴 AAM 系統，而非直接訪問 ChromaDB
...
sequenceDiagram
    ...
    alt 策略 == VECTOR_FIRST
        HRS->>VR: 執行向量檢索
        VR->>ChromaDB: query_vectors(query, top_k)
        ChromaDB-->>VR: 向量檢索結果
        ...
        par 並行執行
            HRS->>VR: 向量檢索任務
            VR->>ChromaDB: query_vectors(query, top_k*2)
            ChromaDB-->>VR: 向量檢索結果
            VR-->>HRS: vector_results
        and
            HRS->>GR: 圖譜檢索任務
...
- ⚠️ **注意**：當前實現依賴 AAM 系統，不是直接使用 ChromaDB。向量檢索通過 `RealtimeRetrievalService` 進行，該服務內部可能使用 ChromaDB 或其他向量存儲。
...
retrieval_manager = RetrievalManager(
    ...
    aam_hybrid_rag=hybrid_rag  # 可選參數
)
...
| 方面 | 文檔描述 | 實際實現 | 差異 |
|------|---------|---------|------|
| **數據庫** | ChromaDB | AAM 系統（RealtimeRetrievalService） | 實現依賴 AAM 系統，而非直接訪問 ChromaDB |
| **檢索方式** | 直接查詢 ChromaDB | 通過 RealtimeRetrievalService | 抽象層不同 |
