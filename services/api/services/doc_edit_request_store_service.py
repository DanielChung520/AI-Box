"""
代碼功能說明: 文件編輯（Doc Edit）request store（Redis 優先 + memory fallback，含 abort flag）
...
class DocEditRequestStoreService:
    """Doc Edit request_id store（MVP）。"""
    ...
    def __init__(self, *, ttl_seconds: int = 60 * 60 * 24) -> None:
        ...
        try:
            ...
            logger.warning("doc_edit_store_redis_unavailable", error=str(exc))

        self._lock = Lock()
        self._fallback: dict[str, DocEditRequestRecord] = {}
        self._fallback_abort: dict[str, bool] = {}

    @staticmethod
    def _request_key(request_id: str) -> str:
        return f"doc:edit:request:{request_id}"

    @staticmethod
    def _abort_key(request_id: str) -> str:
        return f"doc:edit:abort:{request_id}"

    def create(self, record: DocEditRequestRecord) -> None:
        ...
        if self._redis is not None:
            try:
                ...
                logger.warning(
                    "doc_edit_store_redis_set_failed",
    ...
    def get(self, *, request_id: str) -> Optional[DocEditRequestRecord]:
        ...
        if self._redis is not None:
            try:
                ...
                if not isinstance(raw, str):
                    raw = str(raw)
                data = json.loads(raw)
                return DocEditRequestRecord.model_validate(data)
            except Exception as exc:  # noqa: BLE001
                logger.warning(
                    "doc_edit_store_redis_get_failed",
    ...
    def update(
        ...
        *,
        request_id: str,
        status: Optional[DocEditStatus] = None,
        record: Optional[DocEditRequestRecord] = None,
        error_code: Optional[str] = None,
        error_message: Optional[str] = None,
    ) -> Optional[DocEditRequestRecord]:
    ...
    def set_abort(self, *, request_id: str) -> None:
        ...
        if self._redis is not None:
            try:
                ...
                logger.warning(
                    "doc_edit_store_redis_abort_set_failed",
                    request_id=request_id,
                    error=str(exc),
                )
    ...
    def is_aborted(self, *, request_id: str) -> bool:
        ...
        if self._redis is not None:
            try:
                ...
                logger.warning(
                    "doc_edit_store_redis_abort_get_failed",
                    request_id=request_id,
                    error=str(exc),
                )
...
_service: Optional[DocEditRequestStoreService] = None


def get_doc_edit_request_store_service() -> DocEditRequestStoreService:
    global _service
    if _service is None:
        _service = DocEditRequestStoreService()
    return _service


def reset_doc_edit_request_store_service() -> None:
    global _service
    _service = None
