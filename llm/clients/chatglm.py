# 代碼功能說明: ChatGLM 客戶端實現
# 創建日期: 2025-12-30
# 創建人: Daniel Chung
# 最後修改日期: 2026-01-24 23:38 UTC+8

"""ChatGLM 客戶端實現，整合智譜 AI ChatGLM API。"""

from __future__ import annotations

import logging
import os
from typing import Any, AsyncGenerator, Dict, List, Optional

try:
    from openai import AsyncOpenAI
    from openai.types.chat import ChatCompletion, ChatCompletionChunk
    from openai.types.completion import Completion
    from openai.types.embedding import Embedding
except ImportError:
    AsyncOpenAI = None  # type: ignore[assignment, misc]
    ChatCompletion = None  # type: ignore[assignment, misc]
    ChatCompletionChunk = None  # type: ignore[assignment, misc]
    Completion = None  # type: ignore[assignment, misc]
    Embedding = None  # type: ignore[assignment, misc]

from system.infra.config.config import get_config_section

from .base import BaseLLMClient

logger = logging.getLogger(__name__)


class ChatGLMClientError(Exception):
    """ChatGLM 客戶端基礎錯誤。"""


class ChatGLMClient(BaseLLMClient):
    """ChatGLM 客戶端實現（使用 OpenAI 兼容 API）。"""

    def __init__(
        self,
        api_key: Optional[str] = None,
        base_url: Optional[str] = None,
        default_model: Optional[str] = None,
        timeout: Optional[float] = None,
        **kwargs: Any,
    ):
        """
        初始化 ChatGLM 客戶端。

        Args:
            api_key: ChatGLM API 密鑰（從環境變數或配置讀取）
            base_url: API 基礎 URL（可選，默認使用智譜 AI 端點）
            default_model: 默認模型名稱
            timeout: 請求超時時間（秒，可選，從配置讀取）
            **kwargs: 其他參數（兼容性保留）
        """
        if AsyncOpenAI is None:
            raise ImportError(
                "OpenAI SDK is not installed. Please install it with: pip install openai"
            )

        # 從環境變數或配置讀取 API 密鑰
        if api_key is None:
            api_key = os.getenv("CHATGLM_API_KEY") or os.getenv("ZHIPU_API_KEY")
            if not api_key:
                # 嘗試從 LLMProviderConfigService 獲取
                try:
                    from services.api.models.llm_model import LLMProvider
                    from services.api.services.llm_provider_config_service import (
                        get_llm_provider_config_service,
                    )

                    config_service = get_llm_provider_config_service()
                    api_key = config_service.get_api_key(LLMProvider.CHATGLM)
                except Exception as e:
                    logger.debug(f"Failed to get API key from config service: {e}")

                if not api_key:
                    config = get_config_section("llm", "chatglm", default={}) or {}
                    api_key = config.get("api_key") or os.getenv("CHATGLM_API_KEY")

        if not api_key:
            raise ValueError("ChatGLM API key is required")

        # 從配置讀取其他參數
        config = get_config_section("llm", "chatglm", default={}) or {}
        if base_url is None:
            base_url = config.get("base_url", "https://open.bigmodel.cn/api/paas/v4")
        if default_model is None:
            default_model = config.get("default_model", "glm-4")
        if timeout is None:
            timeout = float(config.get("timeout", 60.0))

        self.api_key = api_key
        self.base_url = base_url
        self._default_model = default_model
        self.timeout = timeout

        # 創建 OpenAI 兼容客戶端（智譜 AI 使用 OpenAI 兼容 API）
        client_kwargs: Dict[str, Any] = {
            "api_key": self.api_key,
            "base_url": self.base_url,
            "timeout": self.timeout,
        }

        self._client = AsyncOpenAI(**client_kwargs)

    @property
    def provider_name(self) -> str:
        """返回提供商名稱。"""
        return "chatglm"

    @property
    def default_model(self) -> str:
        """返回默認模型名稱。"""
        return self._default_model

    async def generate(
        self,
        prompt: str,
        *,
        model: Optional[str] = None,
        temperature: Optional[float] = None,
        max_tokens: Optional[int] = None,
        **kwargs: Any,
    ) -> Dict[str, Any]:
        """
        生成文本。

        Args:
            prompt: 輸入提示詞
            model: 模型名稱（可選，使用默認模型）
            temperature: 溫度參數
            max_tokens: 最大 token 數
            **kwargs: 其他參數

        Returns:
            生成結果字典，包含 'text' 或 'content' 字段
        """
        model = model or self.default_model

        try:
            response: Completion = await self._client.completions.create(
                model=model,
                prompt=prompt,
                temperature=temperature,
                max_tokens=max_tokens,
                **kwargs,
            )

            # 提取文本內容
            text = response.choices[0].text if response.choices else ""

            # 構建返回結果
            result: Dict[str, Any] = {
                "text": text,
                "content": text,
                "model": model,
            }

            # 添加 token 使用量統計
            if hasattr(response, "usage") and response.usage:
                result["usage"] = {
                    "prompt_tokens": response.usage.prompt_tokens,
                    "completion_tokens": response.usage.completion_tokens,
                    "total_tokens": response.usage.total_tokens,
                }

            return result

        except Exception as e:
            logger.error(f"ChatGLM generate error: {e}", exc_info=True)
            raise ChatGLMClientError(f"Failed to generate text: {e}") from e

    async def chat(
        self,
        messages: List[Dict[str, Any]],
        *,
        model: Optional[str] = None,
        temperature: Optional[float] = None,
        max_tokens: Optional[int] = None,
        **kwargs: Any,
    ) -> Dict[str, Any]:
        """
        對話生成。

        Args:
            messages: 消息列表，每個消息包含 'role' 和 'content'
            model: 模型名稱（可選）
            temperature: 溫度參數
            max_tokens: 最大 token 數
            **kwargs: 其他參數

        Returns:
            對話結果字典，包含 'content' 或 'message' 字段
        """
        model = model or self.default_model

        try:
            response: ChatCompletion = await self._client.chat.completions.create(
                model=model,
                messages=messages,  # type: ignore[arg-type]
                temperature=temperature,
                max_tokens=max_tokens,
                **kwargs,
            )

            # 提取消息內容
            if response.choices and len(response.choices) > 0:
                message = response.choices[0].message
                content = message.content if message.content else ""
            else:
                content = ""

            # 構建返回結果
            result: Dict[str, Any] = {
                "content": content,
                "message": content,
                "model": model,
            }

            # 添加 token 使用量統計
            if hasattr(response, "usage") and response.usage:
                result["usage"] = {
                    "prompt_tokens": response.usage.prompt_tokens,
                    "completion_tokens": response.usage.completion_tokens,
                    "total_tokens": response.usage.total_tokens,
                }

            return result

        except Exception as e:
            logger.error(f"ChatGLM chat error: {e}", exc_info=True)
            raise ChatGLMClientError(f"Failed to chat: {e}") from e

    async def chat_stream(
        self,
        messages: List[Dict[str, Any]],
        *,
        model: Optional[str] = None,
        temperature: Optional[float] = None,
        max_tokens: Optional[int] = None,
        **kwargs: Any,
    ) -> AsyncGenerator[Dict[str, Any], None]:
        """
        流式對話生成。

        Args:
            messages: 消息列表
            model: 模型名稱（可選）
            temperature: 溫度參數
            max_tokens: 最大 token 數
            **kwargs: 其他參數

        Yields:
            流式生成結果字典
        """
        model = model or self.default_model

        try:
            stream = await self._client.chat.completions.create(
                model=model,
                messages=messages,  # type: ignore[arg-type]
                temperature=temperature,
                max_tokens=max_tokens,
                stream=True,
                **kwargs,
            )

            async for chunk in stream:
                if isinstance(chunk, ChatCompletionChunk):
                    if chunk.choices and len(chunk.choices) > 0:
                        delta = chunk.choices[0].delta
                        if delta and delta.content:
                            yield {
                                "content": delta.content,
                                "model": model,
                                "finish_reason": chunk.choices[0].finish_reason,
                            }

        except Exception as e:
            logger.error(f"ChatGLM chat_stream error: {e}", exc_info=True)
            raise ChatGLMClientError(f"Failed to stream chat: {e}") from e

    async def embeddings(
        self,
        text: str,
        *,
        model: Optional[str] = None,
        **kwargs: Any,
    ) -> List[float]:
        """
        生成文本嵌入向量。

        Args:
            text: 輸入文本
            model: 嵌入模型名稱（可選）
            **kwargs: 其他參數

        Returns:
            嵌入向量列表
        """
        model = model or "text-embedding-ada-002"  # 默認嵌入模型

        try:
            response = await self._client.embeddings.create(
                model=model,
                input=text,
                **kwargs,
            )

            if response.data and len(response.data) > 0:
                embedding = response.data[0]
                if isinstance(embedding, Embedding):
                    return embedding.embedding
                return []

            return []

        except Exception as e:
            logger.error(f"ChatGLM embeddings error: {e}", exc_info=True)
            raise ChatGLMClientError(f"Failed to generate embeddings: {e}") from e

    async def verify_connectivity(self) -> tuple[bool, str]:
        """
        驗證與 ChatGLM API 的連通性。
        Returns:
            (是否成功, 消息)
        """
        try:
            # ChatGLM 是 OpenAI 兼容的，可以嘗試獲取模型列表
            await self._client.models.list()
            return True, "連通性正常"
        except Exception as exc:
            logger.error(f"ChatGLM connectivity check failed: {exc}")
            return False, f"連通性驗證失敗: {str(exc)}"
