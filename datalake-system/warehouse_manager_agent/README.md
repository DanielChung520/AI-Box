...
庫管員Agent（Warehouse Manager Agent）是一個外部業務Agent，作為獨立服務註冊到AI-Box，負責庫存管理業務邏輯：
...
├── __init__.py
├── agent.py                   # WarehouseManagerAgent主類
├── models.py                  # 數據模型
...
```bash
./scripts/warehouse_manager_agent/quick_start.sh
```

### 3. 查看狀態

```bash
./scripts/warehouse_manager_agent/status.sh
```
...
```bash
# 查看所有日誌
./scripts/warehouse_manager_agent/view_logs.sh

# 查看錯誤日誌
./scripts/warehouse_manager_agent/view_logs.sh error
```

### 5. 停止服務

```bash
./scripts/warehouse_manager_agent/stop.sh
```
...
```bash
cd /Users/daniel/GitHub/AI-Box/datalake-system
pytest tests/warehouse_manager_agent/ -v
```

## 相關文檔

- [庫管員-Agent-規格書.md](../../docs/系统设计文档/核心组件/Agent平台/庫管員-Agent-規格書.md)
- [Agent-開發規範.md](../../docs/系统设计文档/核心组件/Agent平台/Agent-開發規範.md)
- [Data-Agent-規格書.md](../../docs/系统设计文档/核心组件/Agent平台/Data-Agent-規格書.md)
