...
本文檔描述的 API 遵循 **MAI-S0112-AI-Box意圖分類協議標準**。

**參考文檔**：`/docs/02-SRS-規格書/MAI-S0112-AI-Box意圖分類協議標準.md`

**協議要點**：
- Request: `nlq`, `intent`, `params`, `action_plan`
- Response: `status`, `result`, `decision_action`, `errors`, `warnings`
- 支持 `forced_intent` 外部意圖傳入
---
...
#### 3.1.1 decision_action 說明

| 值 | 說明 | 發生時機 |
|------|------|----------|
| `EXECUTE` | 正常執行 | SQL 生成成功且執行成功 |
| `PRE_REJECT` | 前置異常 | SQL 生成前驗證失敗（參數缺失、格式錯誤、範圍過大） |
...
| 錯誤碼 | 說明 |
|--------|------|
| `INTENT_UNCLEAR` | 意圖不明確 |
| `MISSING_REQUIRED_PARAM` | 缺少必需參數 |
| `INVALID_PARAM_FORMAT` | 參數格式錯誤 |
| `QUERY_SCOPE_TOO_LARGE` | 查詢範圍過大 |

**後置錯誤 (POST_REJECT)**：

| 錯誤碼 | 說明 |
|--------|------|
| `SCHEMA_NOT_FOUND` | 表格不存在 |
| `COLUMN_NOT_FOUND` | 欄位不存在 |
| `AMBIGUOUS_REFERENCE` | 維度模糊 |
| `BINDER_ERROR` | 綁定錯誤 |
| `NO_DATA_FOUND` | 查無資料 |
| `QUERY_ERROR` | SQL 執行錯誤 |

#### 3.1.3 forced_intent 支援

Data-Agent 支持外部傳入的意圖和參數（MAI-S0112 協議）：
...
當 `task_data.intent` 存在時，Resolver 會使用該意圖進行 SQL 生成，跳過意圖識別步驟。
