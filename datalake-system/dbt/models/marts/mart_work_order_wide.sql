...
{{
    config(
        materialized='table',
        alias='mart_work_order_wide',
        pre_hook="CREATE SEQUENCE IF NOT EXISTS seq_mart_work_order"
    )
}}
...
SELECT
    {{ dbt_utils.generate_surrogate_key(['item_no', 'customer_no', 'status']) }} AS work_order_key,
    item_no,
    customer_no,
    customer_name,
    customer_short_name,
