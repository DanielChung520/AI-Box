...
{{
    config(
        materialized='table',
        alias='mart_inventory_wide',
        pre_hook="CREATE SEQUENCE IF NOT EXISTS seq_mart_inventory"
    )
}}
...
SELECT
    {{ dbt_utils.generate_surrogate_key(['item_no', 'warehouse_no']) }} AS inventory_key,
    item_no,
    warehouse_no,
    location_no,
    unit,
