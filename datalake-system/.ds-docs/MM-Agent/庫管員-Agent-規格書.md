...
- [ ] 1. 更新 `main.py` - FastAPI + MCP 入口
- [ ] 2. 新增 `chain/mm_agent_chain.py` - LangChain 對話鏈
- [ ] 3. 強化 `semantic_analyzer.py` - L1 語義理解（集成 Ollama）
- [ ] 4. 新增 `translator.py` - L2 轉譯引擎（集成 Ollama）
- [ ] 5. 新增 `positive_list.py` - L4 正面表列
- [ ] 6. 新增 `mcp_handler.py` - MCP Handler
- [ ] 7. 新增 `requirements.txt` - 依賴管理
- [ ] 8. 新增 `Dockerfile` - 容器化部署
- [ ] 9. 更新 `api_server.py` - 對接 MM-Agent
- [ ] 10. 測試 - 100 個查詢場景
