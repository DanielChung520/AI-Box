# 代碼功能說明: DT-Agent 配置模組（從 data_agent schema_driven_query/config.py 移植）
# 創建日期: 2026-03-11
# 創建人: Daniel Chung
# 最後修改日期: 2026-03-11
...
- 環境變數覆寫
- 多來源配置
- dt_agent/config/ 優先，fallback 到 metadata_path
"""
...
class EmbeddingConfig(BaseSettings):
    ...
    class Config:
        env_prefix = "DT_AGENT_EMBEDDING_"


class SchemaDrivenQueryConfig(BaseSettings):
    """DT-Agent 主配置"""

    # Metadata 路徑（fallback 用）
    ...
    class Config:
        env_prefix = "DT_AGENT_"

    @property
    def dt_config_dir(self) -> Path:
        """dt_agent/config/ 目錄路徑"""
        return Path(__file__).resolve().parents[2] / "config"

    @property
    def concepts_path(self) -> Path:
        """Concepts JSON 路徑（優先 dt_config_dir，fallback metadata_path）"""
        dt_path = self.dt_config_dir / "concepts.json"
        if dt_path.exists():
            return dt_path
    ...
    def intents_path(self) -> Path:
        """Intents JSON 路徑（優先 dt_config_dir，fallback metadata_path）"""
        dt_path = self.dt_config_dir / "intents.json"
        if dt_path.exists():
            return dt_path
    ...
    def bindings_path(self) -> Path:
        """Bindings JSON 路徑（優先 dt_config_dir，fallback metadata_path）"""
        dt_path = self.dt_config_dir / "bindings.json"
        if dt_path.exists():
            return dt_path
    ...
    def schema_path(self) -> Path:
        """Schema YAML 路徑（優先 dt_config_dir，fallback metadata_path）"""
        dt_path = self.dt_config_dir / f"{self.system_id}.yml"
        if dt_path.exists():
            return dt_path
