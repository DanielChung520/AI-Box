# 代碼功能說明: DT-Agent LLM SQL 生成服務 — 動態 schema + sqlglot 驗證 + 分層 fallback（從 data_agent simple_llm_sql.py 移植）
# 創建日期: 2026-03-11
# 創建人: Daniel Chung
# 最後修改日期: 2026-03-11
...
DEFAULT_MODEL = os.environ.get(
    "OLLAMA_DEFAULT_MODEL", os.environ.get("MOE_CHAT_MODEL", "mistral-nemo:12b")
)
...
2. 只使用上述表格的實際欄位名稱（注意大小寫）
3. 只使用 SELECT 語句（只讀）
4. **嚴格限制輸出格式**：回覆中只能包含一條 SQL 語句，禁止輸出任何解釋、說明文字或 markdown 標記。不要寫「Here is」「以下是」等前綴。
5. **所有表格都是 DuckDB 內建 mart 表**：直接使用表格名稱查詢，例如 `SELECT col FROM mart_inventory_wide WHERE ...`
6. **嚴禁使用 read_parquet() 或 S3 路徑**：不要使用 read_parquet()、s3:// 等語法
...
12. **欄位名稱必須完全匹配**：使用表格中列出的確切欄位名稱，不要自行拼接或猜測欄位名

## 查詢類型判斷：
- 「明細/詳細/詳情/detail」→ **明細查詢（重要！）**：必須 SELECT 該表的**所有欄位**（所有 DIMENSION + METRIC），加 WHERE 條件 + LIMIT。用戶說「明細」意味著要看完整資訊，不要省略任何欄位。
- 「列出/前N筆/查詢XX的資料」→ 一般查詢：SELECT 與查詢意圖相關的主要欄位（不需要全部，但至少包含主要維度和指標）+ WHERE 條件 + LIMIT
