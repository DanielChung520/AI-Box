# 代碼功能說明: DT_IntentRAG 意圖匹配（Qdrant + in-memory fallback）
# 創建日期: 2026-03-11
# 創建人: Daniel Chung
# 最後修改日期: 2026-03-11
...
class DT_IntentRAG:
    _instance: Optional["DT_IntentRAG"] = None
    ...
    def __init__(
        ...
        if intents_file is None:
            ...
            config = get_config()
            intents_file = str(config.intents_path)
        ...
        try:
            ...
            logger.info("DT_IntentRAG Qdrant ready", collection="jp_intents")
        except Exception as exc:
            self._qdrant_available = False
            logger.warning("DT_IntentRAG Qdrant unavailable at init", error=str(exc))
    ...
    def load_intents(self) -> None:
        ...
        try:
            ...
            logger.info(
                "DT_IntentRAG intents loaded", count=len(self._intents), path=self.intents_file
            )
        except Exception as exc:
    ...
    @classmethod
    async def get_instance(cls) -> "DT_IntentRAG":
        if cls._instance is None:
            cls._instance = cls()
        return cls._instance

    @classmethod
    def get_instance_sync(cls) -> "DT_IntentRAG":
        if cls._instance is None:
            cls._instance = cls()
        return cls._instance
