# 代碼功能說明: Markdown 轉 HTML Agent 主類
# 創建日期: 2026-01-23
# 創建人: Daniel Chung
# 最後修改日期: 2026-01-23

"""Markdown 轉 HTML Agent 主類

實現 AgentServiceProtocol 接口，提供 Markdown 到 HTML 的轉換服務。
"""
...
class MdToHtmlAgent(AgentServiceProtocol):
    """Markdown 轉 HTML Agent

    提供 Markdown 到 HTML 的轉換服務。
    """

    def __init__(self):
        """初始化 Markdown 轉 HTML Agent"""
        self.agent_id = "md-to-html"
        self.logger = logger
    ...
    async def execute(self, request: AgentServiceRequest) -> AgentServiceResponse:
        ...
        try:
            ...
            if not conversion_config_data:
                # 允許使用默認配置
                conversion_config_data = {
                    "conversion_id": str(uuid4()),
                    "output_file_name": "converted.html",
                }
            ...
            file_metadata = self.workspace_integration.create_file_from_path(
                task_id=task_id,
                user_id=user_id,
                file_name=conversion_config.output_file_name or f"{output_file_id}.html",
            ...
            conversion_response = ConversionResponse(
                ...
                status="success",
                message="HTML conversion completed successfully",
                metadata={
                    "file_size": file_metadata.file_size,
                },
                audit_info={
                    "converted_at": datetime.utcnow().isoformat(),
                    "converted_by": "md-to-html-v1.0",
                    "tool_version": "pandoc",
                },
            )
            ...
            self.logger.info(
                f"Markdown to HTML conversion completed: task_id={request.task_id}, "
            ...
            self.logger.error(
                f"Markdown to HTML conversion failed: task_id={request.task_id}, error={str(e)}",
                exc_info=True,
            )
            return AgentServiceResponse(
                task_id=request.task_id,
    ...
    async def get_capabilities(self) -> Dict[str, Any]:
        ...
        """
        return {
            "name": "Markdown to HTML Agent (v1.0)",
            "description": "將 Markdown 文件轉換為高品質 HTML 文件，支援 Mermaid 圖表",
            "capabilities": [
                "document_conversion",
                "markdown_to_html",
                "html_generation",
